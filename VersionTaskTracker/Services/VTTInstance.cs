...
public class VTTInstance : IStorable<VTTInstance>
{
    ...
    public IDirectoryInfo WorkingDirectory
    {
        get { return this._workingDirectory; }
    }
    public TasksDbContext TasksDbContext
    {
        get { return this._tasks; }
    }
    public VTTInstanceConfig InstanceConfiguration
    {
        get { return this._config; }
    }
    public List<string> VTTIgnore
    {
        get { return this._vttIgnore; }
    }
    public VTTInstanceMetadata Metadata
    {
    ...
    )
    {
        ...
        string tasksDbPath = Path.Combine(
            workingDirectory.FullName,
            _config.VTTInstanceDirName!,
            _config.VTTInstanceTasksDatabaseName!
        );
    ...
    public void InitializeNew()
    {
        ...
        string tasksDbPath = Path.Combine(
            this._workingDirectory.FullName,
            _config.VTTInstanceDirName!,
            _config.VTTInstanceTasksDatabaseName!
        );
    ...
    private VTTInstance(
        ...
        VTTInstanceConfig iconfig,
        VTTInstanceMetadata metadata,
        VTTInstanceState instanceState,
        List<string> vttIgnore,
        IDirectoryInfo workingDirectory
    )
    {
        _tasks = tasks;
    ...
    public void Load()
    {
        ...
        this._vttIgnore = LoadIgnores();
        this._metadata.Load();
        this._tasks = new TasksDbContext(
            Path.Combine(
                this._workingDirectory.FullName,
                this._config.VTTInstanceDirName!,
                this._config.VTTInstanceTasksDatabaseName!
            ),
            this._workingDirectory.FullName
        );
    }
    ...
    {
        string ignoreFile = Path.Combine(
            this._workingDirectory.FullName,
            this._config.VTTInstanceIgnoreFile!
        );
        ...
        return File.Exists(ignoreFile)
            ? File.ReadAllLines(ignoreFile)
                .Select(l => l.Trim())
                .Where(l =>
                    !string.IsNullOrEmpty(l) && !string.IsNullOrWhiteSpace(l) && !l.StartsWith("#")
                )
                .Select(l =>
                {
                    if (l.Contains("#"))
...
public class VTTInstanceMetadata : IStorable<VTTInstanceMetadata>
{
    ...
    public VTTInstanceMetadata(
        VTTInstanceConfig config,
    ...
    public void Load()
    {
        string metadataFilePath = Path.Combine(
            this._workingDirectory.FullName,
            this._config.VTTInstanceDirName!,
            this._config.VTTInstanceMetadataFile!
        );

        VTTInstanceMetadata metadata = JObject
            .Parse(File.ReadAllText(metadataFilePath))
            .ToObject<VTTInstanceMetadata>()!;
