...
public class TasksDbContext : DbContext, IStorable<TasksDbContext>
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        ...
        modelBuilder.Entity<Component>().HasIndex(c => c.Path).IsUnique(true);

        modelBuilder
            .Entity<VersionTaskTracker.Model.Tracking.Task>()
            .HasIndex(t => t.Int_Id)
            .IsUnique();
    }
    ...
    {
        Component c = TraverseFromDirectory(
            dir,
            null,
            d =>
            {
                return (!ShouldIgnore(GetRelativePath(dir.FullName, d.FullName) + "/", ignores))
                    && (this.Components.AsEnumerable().Any(c => c.Path.Equals(d.FullName)));
            },
            f =>
            {
                return (!ShouldIgnore(GetRelativePath(dir.FullName, f.FullName), ignores))
                    && (this.Components.AsEnumerable().Any(c => c.Path.Equals(f.FullName)));
            }
        );
    ...
    private Component TraverseFromDirectory(
        IDirectoryInfo dir,
        Component? parent,
        Func<IDirectoryInfo, bool> IsDirIncluded,
        Func<IFileInfo, bool> IsFileIncluded
    )
    {
        Component r =
            (
                this.Components.Any(c =>
                    c.Path.Equals(GetRelativePath(this._workingDirectory, dir.FullName) + "/")
                )
            )
                ? this
        ...
        IDirectoryInfo[] dirs = dir.GetDirectories().Where(d => IsDirIncluded(d)).ToArray();

        IFileInfo[] files = dir.GetFiles().Where(f => IsFileIncluded(f)).ToArray();
