...
{
    [CommandParameter(0, Name = "Target", Description = "The Target Component's Path", IsRequired = true)]
    ...
    [CommandParameter(3, Name = "Description",Description = "Task's Description",IsRequired = false)]
    ...
    public ValueTask ExecuteAsync(IConsole console)
    {
        return WhenInstanceReady(console,(c,instance) =>
        {
            ...
            Component? target = instance.TasksDbContext.Components.FirstOrDefault(x => x.Path.Equals(TargetPath));
            
            if(target == null) {
                console.Output.WriteLine($"Component '{TargetPath}' Doesn't exist");
                return ValueTask.CompletedTask;
            }
            var maxIntId = instance.TasksDbContext.Tasks
                .OrderByDescending(t => t.Int_Id)
                .FirstOrDefault()?.Int_Id ?? 0;
            ...
            VersionTaskTracker.Model.Tracking.Task task = instance.TasksDbContext.Tasks.FirstOrDefault(x => x.Id.Equals(id))!;
